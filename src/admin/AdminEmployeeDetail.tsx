    ...
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8 md:px-10">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          ...
          {location ? <><div className="h-48 overflow-hidden rounded-xl sm:h-56"><MapContainer center={[location.lat, location.lng]} zoom={16} className="h-full w-full"><TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />{location.accuracy && <Circle center={[location.lat, location.lng]} radius={location.accuracy} pathOptions={{ color: "#3f8f5f", fillColor: "#3f8f5f", fillOpacity: 0.12, weight: 1 }} />}<Marker position={[location.lat, location.lng]}><Popup>{employee.name}</Popup></Marker></MapContainer></div><p className="mt-3 break-words text-sm text-muted-foreground">{location.lat.toFixed(5)}, {location.lng.toFixed(5)}{location.accuracy ? ` (±${Math.round(location.accuracy)}m)` : ""}</p><p className="text-xs text-muted-foreground">{isLive ? "Live now" : `Last updated ${new Date(location.at).toLocaleString()}`}</p></> : <p className="text-sm text-muted-foreground">No GPS location has been shared yet.</p>}
        </section>
