...
const TELANGANA_EMBLEM =
  "https://www.telangana.gov.in/wp-content/uploads/elementor/thumbs/Telangana-LOGO-q8vapnsrggqk8uafl5x9z4rntchzcznlqb5ngwqga0.png";
...
export default function LoginPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#baf2f7] px-4 py-10 font-sans text-[#003852] sm:px-6">
      ...
      <section className="relative z-10 w-full max-w-[650px] pt-7">
        <div className="rounded-[2rem] border border-white/90 bg-white/95 px-6 pb-11 pt-12 shadow-[0_22px_48px_-18px_rgba(0,55,78,.38)] sm:px-9 sm:pb-12 sm:pt-14">
          <div className="flex items-center justify-center gap-5 sm:gap-8" aria-label="Official Telangana police identity">
            <img src={TELANGANA_EMBLEM} alt="Government of Telangana emblem" className="h-[76px] w-[76px] object-contain sm:h-[88px] sm:w-[88px]" />
          ...
          <header className="mt-9 text-center">
            <p className="text-[11px] font-extrabold tracking-[0.25em] text-[#a77a13] sm:text-xs">GOVERNMENT OF TELANGANA</p>
            <div className="mx-auto mt-3 h-[3px] w-12 bg-[#c9a52d]" />
            <h1 className="mt-5 text-[30px] font-black leading-none tracking-[0.015em] text-[#003852] sm:text-[38px]">HYDERABAD CITY POLICE</h1>
            <p className="mt-2 text-[17px] font-bold tracking-[0.13em] text-[#4e5864] sm:text-[21px]">COMMISSIONERATE</p>
          </header>

          <div className="mx-auto mt-10 flex h-[104px] w-[104px] items-center justify-center rounded-full border border-[#d4dce3] bg-[#f1f5f8] shadow-inner">
          ...
          <form onSubmit={onSubmit} className="mt-6">
            <label className="block">
              ...
              <div className="flex overflow-hidden rounded-xl border-2 border-[#d1dbe2] bg-[#fafcfd] transition focus-within:border-[#00617f] focus-within:ring-4 focus-within:ring-[#00617f]/10">
                <span className="flex min-h-[78px] items-center gap-1.5 border-r border-[#d1dbe2] bg-[#eaf0f4] px-4 text-lg font-extrabold text-[#003852] sm:px-6 sm:text-xl"><Smartphone className="h-5 w-5" />+91</span>
                <input autoFocus inputMode="numeric" maxLength={10} value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, ""))} className="min-w-0 flex-1 bg-transparent px-4 text-xl font-semibold tracking-[0.08em] text-[#003852] outline-none placeholder:font-normal placeholder:tracking-normal placeholder:text-slate-400 sm:px-6 sm:text-2xl" placeholder="Mobile number" aria-label="Mobile number" />
              </div>
            </label>
            ...
            <button type="submit" disabled={busy} className="mt-7 flex min-h-[78px] w-full items-center justify-center gap-2 rounded-xl bg-[#003852] px-4 text-xl font-extrabold tracking-[0.08em] text-white shadow-lg shadow-[#003852]/20 transition hover:bg-[#00506f] focus:outline-none focus:ring-4 focus:ring-[#00617f]/25 disabled:cursor-not-allowed disabled:opacity-60">
              {busy ? "OPENING ACCESS…" : "LOGIN"}<ArrowRight className="h-6 w-6" />
            </button>
          </form>
        </div>
        <p className="mt-8 text-center text-base font-extrabold tracking-[0.18em] text-[#003852] sm:text-xl">DUTY <span className="mx-1 text-[#b38d1a]">•</span> HONOUR <span className="mx-1 text-[#b38d1a]">•</span> COMPASSION</p>
      </section>
    </main>
  );
}
